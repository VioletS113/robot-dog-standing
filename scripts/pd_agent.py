...
"""Script to evaluate environment with PD baseline agent and log metrics."""
...
parser = argparse.ArgumentParser(description="PD baseline evaluator for Isaac Lab environments.")
parser.add_argument(
    "--disable_fabric", action="store_true", default=False, help="Disable fabric and use USD I/O operations."
)
parser.add_argument("--num_envs", type=int, default=1, help="Number of environments to simulate (default: 1).")
parser.add_argument("--task", type=str, default="Template-Robotdogstanding-PD-v0", help="Name of the task.")
...
def setup_logging(base_dir: str = "outputs/logs/pd_policy") -> dict:
...
def main():
    """PD baseline evaluator with Isaac Lab environment."""
    ...
    while episode_count < args_cli.num_episodes and simulation_app.is_running():
        ...
        while not done and simulation_app.is_running():
            with torch.no_grad():
                # PD env ignores incoming actions and computes torques internally.
    ...
    print("\n" + "=" * 60)
    print("PD POLICY BASELINE SUMMARY")
    print("=" * 60)
